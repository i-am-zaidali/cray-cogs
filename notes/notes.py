...
from .models import UserNote, ButtonPaginator
...
class Notes(commands.Cog):
            ...
            final.update(
                {
                    guild.id: {
                        member: [
                            ...
                            for note in data["notes"] if data["notes"]
                            ]
    ...
    async def setnote(self, ctx : commands.Context, member:Optional[discord.Member]=None, *, note):
    ...
    async def delnote(self, ctx, member: Optional[discord.Member]=None, id:int=0):
        ...
        try:
            ...
            return await ctx.send(f"That wasn't a valid id. use `{ctx.prefix}notes` to see which note number you wanna remove.")
