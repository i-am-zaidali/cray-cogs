...
async def interaction_check(ctx: commands.Context, interaction: discord.Interaction):
    if not ctx.author.id == interaction.user.id:
        await interaction.response.send_message("You aren't allowed to interact with this bruh. Back Off!", ephemeral=True)
        return False
...
class YesOrNoView(ViewDisableOnTimeout):
    def __init__(self, ctx: commands.Context, yes_response: str = "you have chosen yes.", no_response: str = "you have chosen no.", *, timeout=180):
...
class CategoriesSelect(Select):
    def __init__(self, ctx, categories: List[DonoBank], embeds: Dict[str, discord.Embed], add_all: bool, placeholder: str):
        placeholder = placeholder or "Select a category:"
        options = [discord.SelectOption(label=bank.name, value=bank.name, description=f"{ctx.guild.name}'s donation bank.", emoji="💰") for bank in categories]
        if add_all:
            options.insert(0, discord.SelectOption(label="All", value="all", description="All donation banks.", emoji="💰"))
        self.embeds = embeds
...
class CategorySelectView(ViewDisableOnTimeout):
    def __init__(self, ctx, categories: List[DonoBank], embeds: Dict[str, discord.Embed], add_all_select: bool, *, placeholder:str=None, timeout=180):
        super().__init__(timeout=timeout, ctx=ctx)
        
        self.embeds = embeds
        
        self.add_item(CategoriesSelect(ctx, categories, embeds, add_all_select, placeholder=placeholder))
...
class LastItemButton(PaginatorButton):
    def __init__(self):
        super().__init__(emoji="\N{BLACK RIGHT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}")
...
class FirstItemButton(PaginatorButton):
    def __init__(self):
        super().__init__(emoji="\N{BLACK LEFT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}")
...
class PaginatorSelect(Select):
    def __init__(self, *, placeholder: str = "Select an item:", length: int):
        options = [discord.SelectOption(label=f"{i+1}", value=i, description=f"Go to page {i+1}") for i in range(length)]
        super().__init__(options=options, placeholder=placeholder)
...
class PaginationView(ViewDisableOnTimeout):
    def __init__(
        ...
        buttons_to_add = [FirstItemButton, BackwardButton, PageButton, ForwardButton, LastItemButton] if len(self.contents) > 2 else [BackwardButton, PageButton, ForwardButton] if not len(self.contents) == 1 else []
    ...
    async def edit_message(self, inter: discord.Interaction):
        ...
        self.update_items()
        self.message = await inter.response.edit_message(
            content=content, embed=embed, view=self
        )
