...
from .models import Giveaway, EndedGiveaway, model_from_time, Requirements, GiveawayFlags, get_guild_settings
from .converters import TimeConverter, WinnerConverter, PrizeConverter
...
class Giveaways(commands.Cog):
                ...
                for message_id, giveaway in data.items():
                    ...
                    if giveaway.ended:
                        try:
                            ...
                            log.exception(f"Error occurred while ending a giveaway with message id: {giveaway.message_id}", exc_info=e)
    ...
    @commands.group(name="giveaway", aliases=["g"], invoke_without_command=True, cooldown_after_parsing=True)
    ...
    @g.command(name="start", aliases=["s"], usage="[time] <winners> [requirements] <prize> [flags]")
    async def g_start(
        self,
        ...
        *,
        ...
        starts_in = flags.starts_in if flags.starts_in and flags.starts_in > datetime.now(timezone.utc) else datetime.now(timezone.utc)
    ...
    @g.command(name="end")
    async def g_end(self, ctx: commands.Context, message: discord.Message = None, reason: str = ''):
        """
        ...
        if not guild:
            return await ctx.send("It seems like this server has no giveaways, active or otherwise.")
        
        giveaway = guild.get(message.id)
        if not giveaway:
            return await ctx.send("That message doesnt seem to be a valid giveaway. (or it was but is no longer present in cache)")
        
        if isinstance(giveaway, EndedGiveaway):
            return await ctx.send("That giveaway has already ended. You cannot end an ended giveaway but you can reroll it!")
        ...
        try:
            ...
            log.exception(f"Error occurred while ending a giveaway with message id: {giveaway.message_id}", exc_info=e)
            return await ctx.send(f"There was an error while ending this giveaway: \n {box(e, 'py')}")
        
        await ctx.tick(message="Giveaway was ended successfully!")
        
    @g.command(name="reroll")
    async def g_reroll(self, ctx: commands.Context, message: discord.Message = None, winners: WinnerConverter = 1):
        ...
        if isinstance(giveaway, Giveaway):
            return await ctx.send("That giveaway is still running. It needs to end in order to be rerolled!")
        
        try:
            await giveaway.reroll(ctx, winners)
            
        except Exception as e:
            log.exception(f"Error occurred while rerolling a giveaway with message id: {giveaway.message_id}", exc_info=e)
            return await ctx.send(f"There was an error while rerolling this giveaway: \n {box(e, 'py')}")
