...
class Timer(commands.Cog):
    ...
    async def timer_start(self, ctx: commands.Context, time: TimeConverter, *, name: str = "New Timer!"):
        """
        Start a timer.
    ...
    async def timer_list(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="\n".join("{} - {}".format(f"[{x.name}]({x.jump_url})", cf.humanize_timedelta(timedelta=x.remaining_time)) for x in self.cache[ctx.guild.id]),
    ...
    async def tset_showsettings(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description=f"Emoji: `{settings.emoji}`\n"
                        ...
                        (f"\nMax duration: `{cf.humanize_timedelta(seconds=await self.config.max_duration())}`" if await ctx.bot.is_owner(ctx.author) else ""),
        )
