...
class Requirements(commands.Converter):
        ...
        for key, value in self.items():
            if value:
                if isinstance(value, int):
                    ...
                    continue
                final += f"*{key.capitalize()} Roles:*\t{humanize_list([f'<@&{i}>' for i in value])}\n"
        return final
    ...
    def as_role_dict(self) -> Dict[str, Union[List[discord.Role], discord.Role, int]]:
        ...
        for k, v in (org.copy()).items():
            if isinstance(v, list):
                org[k] = [self.verify_role(int(i)) for i in v if self.verify_role(int(i)) is not None]
    ...
    async def convert(cls, ctx, arg:str):
        ...
        roles = {"blacklist": [], "bypass" : [], "required": [], "default_by": [], "default_bl": [], "amari_level": None, "amari_weekly": None}
        ...
        if isinstance(maybeid, list):
            for i in maybeid:
                if not "[" in i:
                    ...
                    if not role:
                        raise ValueError("Role with id: {} not found.".format(i))
            ...
            if not "[" in maybeid:
                ...
                if not role:
                    raise ValueError("Role with id: {} not found.".format(maybeid))
                roles["required"].append(role.id)
...
class Giveaway:
    ...
    async def end(self) -> None:
        try:
            ...
            await self.channel.send(f"Can't find message with id: {self.message_id}. Removing id from active giveaways.")
        ...
        channel = msg.channel
        gmsg = msg
        entrants = await gmsg.reactions[gmsg.reactions.index(reduce(lambda x, y: x if str(x.emoji) == self.emoji else y if str(y.emoji) == self.emoji else None, gmsg.reactions))].users().flatten()
