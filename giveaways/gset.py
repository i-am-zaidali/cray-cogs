...
class gsettings(main):
	...
	@commands.group(name="giveawaysettings", aliases=["gset", "giveawaysetting"], invoke_without_command=True)
	...
	@gset.command(name="giveaway_message", aliases=["gmsg"],
	usage="<message>")
	@commands.admin_or_permissions(administrator=True)
	...
	async def winnerdm(self, ctx, status:bool):
		...
		await ctx.reply("The winner will be dm'ed when the giveaway ends now." if status == True else "The winner will not be dm'ed when the giveaway ends.")
	...
	async def hostdm(self, ctx, status:bool):
		...
		await ctx.reply("The host will be dm'ed when the giveaway ends now." if status == True else "The host will not be dm'ed when the giveaway ends.")

	@gset.command(name="end_message", aliases=["endmsg"],
	usage="<message>")
	@commands.admin_or_permissions(administrator=True)
	...
	async def manager(self, ctx, *roles:discord.Role):
		...
		await ctx.reply(f"{humanize_list(role.mention for role in roles)} have been set as the giveaway managers!", allowed_mentions=discord.AllowedMentions(roles=False, replied_user=False))
	...
	async def auto(self, ctx, toggle:bool):
		...
		await ctx.reply("Giveaway commands will automatically delete now." if toggle == True else "Giveaway commands will retain.")
		...
		if not roles:
			...
			return await ctx.send(
				embed=discord.Embed(
					...
					description="\n\n".join([str(role.mention) for role in roles]) if roles else "No roles have been blacklisted from giveaways permanently.",
			...
			return await ctx.send(
				embed=discord.Embed(
					title=f"Role Bypasses for `{ctx.guild.name}`!",
				 	description="\n\n".join([str(role.mention) for role in roles]) if roles else "No role bypasses set in this server.",
		 			color=discord.Color.green()
				)
			)
			...
			return await ctx.send(
				embed=discord.Embed(
					title=f"Role Multipliers for `{ctx.guild.name}`'s giveaways!",
				 	description=box("\n\n".join([f"@{k.name:<10}  {'<'+'-'*15+'>':>5}  {v:>5}" for k, v in roles.items()]) if roles else "No role multipliers set in this server."),
		 			color=discord.Color.green()
				)
			)
