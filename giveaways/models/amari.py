...
class NotFoundError(AmariException):
    ...
    def __init__(self, response: ClientResponse.status, message: typing.Optional[str] = "Requested Guild/User was not found"):
...
class RatelimitedError(AmariException):
    ...
    def __init__(self, response: ClientResponse.status, message: typing.Optional[str] = "You are being ratelimited by the API. Please try again later."):
        super().__init__(response, message)
...
class APIError(AmariException):
    ...
    def __init__(self, response: ClientResponse.status, message: typing.Optional[str] = "The request couldn't be resolved due to an error with the Amari API. Try again later."):
        super().__init__(response, message)
...
class AmariClient:
    ...
    def _find_user(self, l: dict, user_id: int):
        user = list(
            filter(
                lambda member: member.get("id") == str(user_id), l
            )
        )
        if user:
            return user[0]
