...
class FTRView(ViewDisableOnTimeout):
    def __init__(self, ctx: commands.Context, timeout, giveaway):
        ...
        super().__init__(
            timeout=timeout,
            ctx=ctx,
            timeout_message="Nobody reacted in time. This giveaway has ended.",
        )

        self.add_item(JoinGiveawayButton(ctx.bot, self.giveaway.emoji, False, self.clicked))
    ...
    async def clicked(self: JoinGiveawayButton, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content="GIVEAWAY ENDED!", embed=embed, view=self.view
        )

        settings = await get_guild_settings(giveaway.guild_id)

        await interaction.followup.send(
            content=settings.endmsg.format_map(
                {
                    "winner": interaction.user.mention,
                    "prize": giveaway.prize,
                    "link": giveaway.jump_url,
                }
            )
        )
