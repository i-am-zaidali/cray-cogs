...
from .models import Requirements, Giveaway
...
class conf:
    ...
    async def get_all_roles_multi(self, guild:discord.Guild):
        ...
        final = {d[0]: d[1] for d in sorted(final.items(), key=lambda x: x[1], reverse=True) if not d[1] == 0}
        return final
    ...
    async def blacklist_role(self, guild:discord.Guild, roles:list):
        ...
        return f"Blacklisted `{humanize_list([f'`@{role.name}`' for role in roles])}`` permanently from giveaways." + (f"{humanize_list(failed)} were already blacklisted." if failed else "")
    ...
    async def unblacklist_role(self, guild:discord.Guild, roles:list):
        ...
        return f"UnBlacklisted {humanize_list([f'`@{role.name}`' for role in roles])} permanently from giveaways." + (f"{humanize_list(failed)} were never blacklisted" if failed else "")
    ...
    async def unbypass_role(self, guild:discord.Guild, roles:list):
        ...
        return f"Removed giveaway bypass from {humanize_list([f'`@{role.name}`' for role in roles])}." + (f"{humanize_list(failed)} were never allowed to bypass" if failed else "")
    ...
    async def bypass_role(self, guild:discord.Guild, roles:list):
        ...
        return f"Added giveaway bypass to {humanize_list([f'`@{role.name}`' for role in roles])}." + (f"{humanize_list(failed)} were never allowed to bypass" if failed else "")
    ...
    async def all_blacklisted_roles(self, guild:discord.Guild, id_or_object=True):
        async with self.config.guild(guild).blacklist() as bl:
            ...
            return [guild.get_role(_id).id if id_or_object==True else guild.get_role(_id) for _id in bl]
    ...
    async def all_bypass_roles(self, guild:discord.Guild, id_or_object=True):
        async with self.config.guild(guild).bypass() as by:
            ...
            return [guild.get_role(_id).id if id_or_object==True else guild.get_role(_id) for _id in by]
